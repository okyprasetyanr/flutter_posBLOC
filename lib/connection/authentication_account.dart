...
Future<UserCredential?> authenticatorAccount({
  ...
  try {
    if (signup) {
      ...
      final map = data.data()!;
      if (await checkUserById_Isar(uidUser) == false) {
        await saveAccount_Isar(
          ModelUser(
            idUser: uidUser,
            statusUser: StatusDataX.fromString(
              map[FieldDataUser.status_user.name],
            ),
            ...
            permissionsUser: {
              for (final permission in Permission.values)
                permission:
                    StatusDataX.fromString(
                          map[FieldDataUser.status_user.name],
                        ) ==
                        StatusData.Aktif
                    ? map[FieldDataUser.permissions_user.name][permission
                              .name] ??
                          false
                    : false,
            },
